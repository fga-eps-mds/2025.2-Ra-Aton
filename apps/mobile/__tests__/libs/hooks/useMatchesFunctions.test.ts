...
describe('Hook: useFeedMatches (100% Coverage)', () => {
  ...
  it('Web: deve trocar time direto', async () => {
    ...
    await act(async () => {
      result.current.switchTeam(mockMatchA, jest.fn());
    });
  ...
  it('deve tratar erro genérico (não-axios) ao trocar time', async () => {
    ...
    (switchTeamRequest as jest.Mock).mockRejectedValue(new Error('Erro switch'));

    act(() => { result.current.switchTeam(mockMatchA, jest.fn()); });
    const confirmBtn = (Alert.alert as jest.Mock).mock.calls[0][2][1];
    await act(async () => confirmBtn.onPress());

    expect(Alert.alert).toHaveBeenCalledWith('Erro', expect.stringContaining('Tente novamente'));
  });

  it('deve tratar erro axios genérico ao trocar time', async () => {
    const { result } = renderHook(() => useFeedMatches());
    await waitFor(() => expect(result.current.isLoading).toBe(false));

    (switchTeamRequest as jest.Mock).mockRejectedValue(createAxiosError(500, 'Erro server'));
